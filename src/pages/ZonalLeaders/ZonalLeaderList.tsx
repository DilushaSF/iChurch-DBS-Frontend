...
const ZonalLeaderList: React.FC = () => {
  ...
  const fetchZonalLeaders = async (): Promise<void> => {
    try {
      setLoading(true);
      ...
      setError("Failed to fetch zonal leaders. Please try again.");
  ...
  const filteredZonalLeaders = zonalLeaders.filter(
    (zonalLeader) =>
      zonalLeader.firstName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      zonalLeader.lastName.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <Paper
      ...
      <Box sx={{maxWidth: "1400px", margin: "0 auto"}}>
        ...
        <Box
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Manage all zonal leader records
            </Typography>
          </Box>
          <Button
            variant="contained"
        ...
        {/* Zonal Leaders Table */}
        {filteredZonalLeaders.length === 0 ? (
          <Card sx={{p: 6, textAlign: "center"}}>
            <Typography variant="h6" color="text.secondary" gutterBottom>
              No zonal leader records found.
            </Typography>
            <Button
              variant="contained"
              startIcon={<AddIcon />}
          ...
          <TableContainer component={Paper} elevation={1}>
            <Table sx={{minWidth: 900}}>
              ...
              <TableBody>
                {filteredZonalLeaders.map((zonalLeader) => (
                  <TableRow
                    key={zonalLeader._id}
                    hover
                    ...
                    <TableCell>{zonalLeader.address}</TableCell>
                    <TableCell>{formatDate(zonalLeader.dateOfBirth)}</TableCell>
                    <TableCell>{zonalLeader.contactNumber}</TableCell>
                    <TableCell align="center">
                      {zonalLeader.zoneNumber}
                    </TableCell>
                    <TableCell>
                      {formatDate(zonalLeader.appointedDate)}
                    </TableCell>
                    <TableCell align="center">
                      <Box
                        sx={{
...
export default ZonalLeaderList;
