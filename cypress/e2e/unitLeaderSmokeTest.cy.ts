describe("Unit Leader module Smoke Test", () => {
    ...
    it("should add a record, save and redirect to the list view ", () => {
        ...
        cy.visit("/unit-leaders/add");

        const randomId = Math.floor(10 + Math.random() * 90);

        cy.get("input[name='firstName']").type("Test");
        cy.get("input[name='lastName']").type(`Unit Leader-${randomId}`);
        ...
        cy.get("input[name='unitNumber']").type("1");

        cy.get("button[type='submit']").click();
        cy.url().should("include", "/unit-leaders");
        cy.contains("Unit Leaders").should("exist");
    });
